...
Variant workflow: when a product has variants with numeric, coded, untranslated, or otherwise unclear option values, use homestro_read_variants first. Read the complete variant list, selected options, SKU and assigned image for every variant. Then determine the correct German option value only from reliable evidence already available in Shopify or supplied by the merchant. Do not guess a color from a numeric code. If the merchant or verified source provides the mapping, pass it to homestro_optimize_product using option_values. Directly update the variant option values with productVariantsBulkUpdate even when images are assigned. Preserve the existing variant images. Verify the returned selectedOptions after the mutation. Never claim success without verification.
